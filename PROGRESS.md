...
Scope: M4.1, M4.2, M4.3, M4.4, and M4.5. Package the already-reviewed M2/M3 jobs into one async scheduler process; add the daily database keepalive and five-minute host-health jobs; expose an internal FastAPI process with database-aware `/healthz` and Cloudflare-Access-protected `/ops/status`; build a digest-pinned, non-root, read-only-friendly runtime image; and add the four-service Compose definition from `docs/HOSTING.md` Part 3. This is G0 only: change `PROGRESS.md`, open a draft PR, and stop until Rehaan comments `G0 approved` on the final exact plan.

Files to create/modify: create `src/tis/runtime.py`, `src/tis/scheduler.py`, `src/tis/healthcheck.py`, `src/tis/jobs/keepalive.py`, `src/tis/jobs/host_health.py`, `src/tis/api/access.py`, `src/tis/api/status.py`, `src/tis/api/app.py`, `deploy/compose.yaml`, `scripts/check_image.py`, `tests/unit/test_runtime.py`, `tests/unit/test_scheduler.py`, `tests/unit/test_healthcheck.py`, `tests/unit/test_keepalive.py`, `tests/unit/test_host_health.py`, `tests/unit/test_access.py`, `tests/unit/test_api.py`, `tests/unit/test_check_image.py`, `tests/contract/test_compose_contract.py`, `tests/fixtures/compose/synthetic.env`, `docs/runbooks/service-packaging.md`, and `docs/pr-notes/M4.md`; modify `.gitignore`, `.dockerignore`, `.env.example`, `Dockerfile`, `Makefile`, `pyproject.toml`, `uv.lock`, `docs/dependencies.md`, `src/tis/config.py`, `src/tis/http.py`, `src/tis/integrations/google_calendar.py`, `scripts/guard.py`, `tests/unit/test_config.py`, `tests/unit/test_http.py`, `tests/unit/test_google_calendar.py`, `tests/unit/test_guard.py`, `tests/unit/test_betterstack.py`, and `PROGRESS.md`. `.gitignore` will ignore `.env` and `.env.*` while explicitly retaining `.env.example`; the existing user-owned `.env` will not be read, modified, staged, or committed. No GitHub workflow, Terraform, SOPS, secret/ciphertext, database schema/SQL draft, M2/M3 mapping or other integration module, `src/tis/guards.py`, `scripts/verify_bundle.sh`, deployment script, backup implementation, or Cloudflare API call will change.

Protected paths touched: yes (`deploy/compose.yaml`, `src/tis/http.py`, and `scripts/guard.py`), so implementation is a `protected-change`. `deploy/compose.yaml` will contain the production service topology and hardening from `HOSTING.md`; `src/tis/http.py` will add only the exact approved Cloudflare Access HTTPS host; `scripts/guard.py` will add the same host to `ALLOWED_URL_HOSTS` so NET004 continues to require literal parity. `AGENTS.md`, `.github/**`, `infra/**`, `.sops.yaml`, `secrets/**`, `scripts/verify_bundle.sh`, and `src/tis/guards.py` will not change.

New dependencies: four exact direct runtime pins: `APScheduler==3.11.3` (MIT; mature asyncio job scheduling with `max_instances`, coalescing, and misfire handling), `fastapi==0.141.1` (MIT; typed ASGI routing/lifespan and response models), `uvicorn==0.53.0` (BSD-3-Clause; minimal pure-Python ASGI server, without the `standard` extra), and `PyJWT==2.14.0` (MIT; strict RS256 JWT header/claim/signature validation using the already-pinned `cryptography`; it performs no network I/O). Anticipated new locked transitives are `tzlocal==5.4.4` (MIT; APScheduler timezone support), `starlette==1.6.0` (BSD-3-Clause; FastAPI ASGI layer), `annotated-doc==0.0.5` (MIT; FastAPI annotation metadata), and `click==8.5.0` (BSD-3-Clause; Uvicorn CLI). Existing compatible packages satisfy the remaining requirements: `pydantic==2.13.5`, `typing-extensions==4.16.0`, `typing-inspection==0.4.4`, `anyio==4.15.1`, `h11==0.16.0`, and `cryptography==50.0.1`. The lock operation must not introduce FastAPI/Uvicorn extras, WebSockets, reload/watch packages, templating, multipart, YAML, an alternate HTTP client, or a JWT library with network behavior. `docs/dependencies.md` will record every direct addition with license, reason, and alternatives; no development dependency changes.

New egress hosts: exactly `torusmesh.cloudflareaccess.com`, for GET-only retrieval of Access signing keys at `/cdn-cgi/access/certs` and as the only accepted JWT issuer origin; no wildcard Cloudflare team host is allowed. The exact host will be added to runtime and guard lists, and redirects remain subject to `ControlledClient`. The keepalive uses the existing `DATABASE_DSN` psycopg pool, so M4 adds no Supabase REST host or RPC endpoint. No new Zoho, Google, PostHog, Better Stack, Docker, GHCR, R2, LinkedIn, or other service egress entry is added to `tis`; image pulls/build traffic are tooling, not runtime service egress.

New scopes/privileges: none. The existing `tis_service` database role and `DATABASE_DSN` are used for `SELECT 1`, fixed parameterized health/status reads of `leads`, `integration_log`, and `integration_state`, plus existing `integration_state` last-run writes; no Supabase REST call, publishable-key privilege, grant, role, RLS, table, or migration is added. Cloudflare Access validation uses the existing non-secret audience and team domain and does not call an admin API or require a Cloudflare token. The API container receives the same runtime environment described in `HOSTING.md` but exposes no personal data and has no host-network, Docker-socket, privileged, or published-port access.

Environment sample and secret hygiene: `.env.example` will contain names, safe comments, and blank values only; it will never contain a real hostname identifier, email, audience, token, key, DSN, digest, or credential. Its Google block will match M3 exactly: `GOOGLE_PRIVATE_KEY` is absent, `GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY` is the canonical key, and blank `GOOGLE_TOKEN_URL` and `GOOGLE_CALENDAR_API_URL` placeholders are present. M4 will add blank placeholders for `LEAD_SYNC_INTERVAL_SECONDS`, `BOOKING_SYNC_INTERVAL_SECONDS`, `HOST_HEALTH_INTERVAL_SECONDS`, `KEEPALIVE_INTERVAL_SECONDS`, `SCHEDULER_WATCHDOG_PATH`, `SCHEDULER_WATCHDOG_MAX_AGE_SECONDS`, `GHCR_OWNER`, `TIS_DIGEST`, `CLOUDFLARED_DIGEST`, and `BACKUP_DIGEST`; it will not add `SUPABASE_KEEPALIVE_RPC` or `BETTERSTACK_SYNC_URL`. Existing `CLOUDFLARE_ACCESS_TEAM_DOMAIN` and `CLOUDFLARE_ACCESS_AUDIENCE` placeholders remain blank. When M3 loads `GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY`, it will normalize literal escaped newlines with `.replace("\\n", "\n")` immediately before PEM parsing so a raw service-account JSON `private_key` string can be supplied through an environment file without changing its content. Configuration/Google tests will prove the example contains every settings-model field exactly once with an empty value, the legacy key is absent, an entirely synthetic `\\n`-escaped PEM reaches the parser with real newlines, and `.env`, `.env.local`, and `.env.prod` are ignored while `.env.example` remains tracked. Runtime code will neither load nor log arbitrary environment files, and no test will use the user's local `.env`.

External writes added: no new downstream action. The daily keepalive first performs read-only `SELECT 1` through the existing psycopg pool and, only after success, reuses the existing guarded `send_heartbeat` function with `BETTERSTACK_KEEPALIVE_URL`; host health likewise uses the existing host-health heartbeat only when every condition passes. Existing heartbeat budgets remain 10 per run and 400 per UTC day, with keys `<monitor>:<run-id>`. Dry run or kill switch still permits the harmless database read but causes the guarded heartbeat to record `skipped` and issue zero HTTP. Lead and booking jobs receive no M4 heartbeat. Successful job completion writes non-personal `last_run:<job>` timestamps to `integration_state`; this is internal service bookkeeping, not a downstream external write. No Supabase REST/RPC, Google, Zoho, PostHog, Cloudflare, container-registry, backup, or infrastructure write is added.

Scheduler and runtime design: `runtime.py` is the single dependency-assembly boundary for settings, database pool, state/audit stores, `WriteContext`, controlled HTTP, field maps, and M2/M3 job contexts; both entrypoints close all resources on shutdown. `scheduler.py` uses APScheduler's asyncio scheduler in UTC with interval defaults lead=30 seconds, booking=120 seconds, host health=300 seconds, and keepalive=86400 seconds; configuration validators permit disabling jobs but never scheduling faster than those minimums. Every job uses `max_instances=1`, `coalesce=true`, and a 60-second misfire grace; the scheduler does not persist job payloads or credentials. A wrapper records redacted job/result counters and last success state, catches at the job boundary so one job cannot stop the loop, and updates a `/tmp` watchdog timestamp; it does not add generic lead/booking heartbeats. The keepalive and host-health jobs alone invoke their approved heartbeat functions after their own success conditions. `healthcheck.py` checks the API role by opening the configured database and executing fixed `SELECT 1`, checks the scheduler role by requiring a recent watchdog timestamp, and has a minimal process/import mode for the Dockerfile-level `HEALTHCHECK`; it never uses an uncontrolled HTTP or socket client.

Keepalive and host-health design: `keepalive.py` executes one trivial `SELECT 1` through the existing `DATABASE_DSN` psycopg pool and calls the existing Better Stack keepalive heartbeat only after that round-trip succeeds; it has no HTTP client, Supabase URL, publishable key, RPC name, response payload, or new egress. `host_health.py` uses `shutil.disk_usage('/host-state')` and fixed parameterized database reads to require disk usage below 85%, `integration_state` key `last_backup_at` below 26 hours, zero unsynced leads older than 30 minutes, zero `google_calendar/sync_booking` errors in the last hour, and kill switch off. It also exposes `last_restore_test_at` in status without making it a five-minute heartbeat condition. Missing/unparseable `last_backup_at` before M5, or an unavailable mount/database, fails closed and suppresses the heartbeat. Results contain only booleans/counts/timestamps/error classes, never lead or attendee fields. The scheduler sends `tis-health` only when every condition passes.

API and Access design: `api/app.py` disables public Swagger/ReDoc/OpenAPI routes, opens/closes shared runtime resources in lifespan, and returns 200 from `/healthz` only when fixed `SELECT 1` succeeds; failure returns a generic 503 without exception/DSN details. `/ops/status` requires `Cf-Access-Jwt-Assertion` and returns 401/403 generically on missing or invalid tokens. `access.py` fetches and caches JWKS only through `ControlledClient`, selects an RSA key by `kid`, permits only `RS256`, validates signature, exact issuer `https://torusmesh.cloudflareaccess.com`, configured audience, expiry, not-before, and issued-at with bounded clock skew, refreshes once for an unknown/rotated key, and fails closed on malformed keys/claims/network errors. `status.py` uses fixed parameterized aggregate queries and typed response models for version/image digest, DRY_RUN, kill switch, each job's last success/error time, today's guarded write counts versus budgets, unsynced/aged lead counts, booking error count, and last backup/restore-test timestamps. It omits emails, names, record ids, note/task text, tokens, headers, DSNs, raw exceptions, and raw payloads.

Image and Compose design: `Dockerfile` becomes a multi-stage build using `python:3.12.14-slim@sha256:2f17fc044b579bab302c2e8054d3a686e2cb9a83de48e70534b94cd8ebbe06a9` for builder/final stages and copies uv from `ghcr.io/astral-sh/uv:0.11.30@sha256:93b61e21202b1dab861092748e46bbd6e0e41dd84f59b9174efd2353186e1b47`; `uv sync --frozen --no-dev` installs the locked environment in the builder. The final stage contains no package manager invocation, compiler, tests, docs, secrets, or root-owned writable application path; it sets `PYTHONDONTWRITEBYTECODE`, runs uid/gid 10001, provides a Dockerfile-level healthcheck, and copies only the virtual environment, source, and non-secret mapping TOML. `scripts/check_image.py` fails unless the configured image is under 250 MB, user is `10001:10001`, a healthcheck exists, and no mutable tag is used as deployment evidence. `deploy/compose.yaml` follows `HOSTING.md` Part 3 exactly for `tis-scheduler`, `tis-api`, `cloudflared`, and `backup`: digest-valued image variables, read-only root filesystems, `cap_drop: [ALL]`, `no-new-privileges`, bounded tmpfs/memory/CPU/logs, non-root application users, restart policy, no privileged/host network/Docker socket, no published ports, `tis-api` exposed only on internal port 8080, and `/host-state` mounted read-only only into the scheduler. M5 still owns backup-image contents and deploy/rollback scripts.

Tests: all HTTP is intercepted, all JWTs/keys/ids/domains are synthetic except the exact public service hostname approved here, and no test contacts Supabase, Cloudflare, Better Stack, Zoho, Google, Docker Hub, or production. Runtime/config tests prove safe defaults, required settings only when jobs/API are enabled, exact host validation, secret redaction, fixed minimum intervals/lookback/budgets, deterministic resource closure, and the exact M3 Google environment contract including escaped-newline PEM normalization. Scheduler tests use injected fake jobs/clock/heartbeat and prove all four schedules, UTC, `max_instances=1`, coalescing, 60-second misfire grace, no overlaps, per-job isolation, last-run recording, no lead/booking heartbeat, keepalive/host-health success-only heartbeat, watchdog updates, and clean shutdown. Keepalive tests inject a fake database pool and heartbeat, proving exactly one `SELECT 1`, heartbeat only after success, database-error isolation, dry-run and kill-switch skipped audit with zero HTTP, existing heartbeat budget/idempotency behavior, and no DSN or result logging. Host-health tests plant every threshold edge and dependency failure separately and prove the heartbeat occurs only when all checks pass with no personal data logged.

Tests continued: Access tests generate ephemeral RSA keys, serve synthetic JWKS with `respx`, and prove valid issuer/audience/signature/expiry acceptance; missing header; wrong issuer/audience/algorithm/key/signature; expired/not-yet-valid tokens; malformed claims/JWKS; cache reuse; one unknown-`kid` refresh; bounded timeout; exact host/redirect denial; and no JWT/header/key logging. API tests inject fake database/status providers and prove `/healthz` 200/503, `/ops/status` 401/403/200, response-model exclusion of personal/raw fields, and no unauthenticated status query. Healthcheck tests cover each role and stale scheduler watchdog. Image/Compose tests and `make compose-config` verify the pinned digests, final uid, healthchecks, image size, four expected services, digest-only image references, hardening controls, networks, mounts, resource limits, absence of ports/privileged/host network/Docker socket, and that `/healthz` is only reachable through the internal network/tunnel. `scripts/guard.py` tests retain NET004 parity and plant an unapproved Access host. `make check`, `make compose-config`, and the image-size check must pass with no skipped/xfail/deleted tests and at least 85% coverage for new jobs/runtime/API code.

Compose acceptance boundary: unit/integration tests will run every scheduler job with injected fake database/HTTP/state dependencies in dry-run mode, and the built local image plus production Compose file will be structurally validated without credentials. Structural Compose validation plus those injected all-job tests is the approved M4 acceptance approach. A fake Compose overlay is explicitly out of scope. A literal `docker compose up` of all four production services is not a credential-free M4 G1 check because `cloudflared` requires a real tunnel token, the M5 backup image does not yet exist, and M3 Calendar discovery performs read-only Google HTTP even when external writes are dry-run. The implementation will not add a test-mode network bypass, fake credential path, mutable image, Docker socket, alternate host, or separate fake overlay.
...
**M3 G3 verification: PASS (Claude, Opus, verification-only) — 2026-09-20**

Reviewed post-merge at `ca4b7ca` (nothing deployed; DRY_RUN default true).

- Egress: `oauth2.googleapis.com` (token) and `www.googleapis.com` (Calendar) added to both `EGRESS_ALLOWLIST` and guard `ALLOWED_URL_HOSTS`; NET004 parity holds. httpx still only in `http.py`.
- `integrations/google_calendar.py`: service-account auth implemented as a locally signed RS256 JWT (cryptography) exchanged via `ControlledClient` at the token URL — no google-auth/google-api-python-client, so nothing bypasses egress control. Scope is `calendar.events.readonly` (read-only). Token cached, refreshed 5 min early, single forced re-auth on 401. events.list is incremental (syncToken), bounded, 410 -> SyncTokenGone handled, strict pydantic validation.
- `jobs/booking_sync.py`: cursor stored in `integration_state` (`google_calendar:handoff_review`), bounded to 10 pages / 500 events. Zoho write is `sync_booking`, `@external_write(source="google_calendar", action="sync_booking", key=event.id)` — guarded and idempotent.
- Zoho additions (search contact/lead, note, task, upsert) exercised by fixtures; write path guarded.
- 174 tests, 88.61% coverage, guard/gitleaks/trivy PASS, CI green.

Config note: M3 renamed the Google env vars. The service now expects `GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY` (not `GOOGLE_PRIVATE_KEY`) and adds `GOOGLE_TOKEN_URL` and `GOOGLE_CALENDAR_API_URL`. `.env.example` must be aligned in M4.

Security hygiene: a real test-meeting link was pasted in chat earlier; the committed fixture is redacted and retained no live values, but recreate/regenerate that test meeting's conferencing details out of caution.

Verdict: PASS. Next gate: M3 G4 dry-run (runs once M4 provides the runner).
