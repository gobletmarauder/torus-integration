...
New scopes/privileges: the minimum Zoho self-client permission for this slice is `ZohoCRM.modules.leads.WRITE`, as required by Zoho's v8 Upsert API; no module-wide `ALL`, delete, Contacts, Notes, Tasks, Campaigns, settings, search, or admin scope is requested. Existing `tis_service` database access is used only to select `public.leads`, read/write `public.integration_state`, and read/upsert `public.integration_log`; no grant, role, schema, RLS, or table change is made. Settings add validated optional names for `ZOHO_CLIENT_ID`, `ZOHO_CLIENT_SECRET`, `ZOHO_REFRESH_TOKEN`, `ZOHO_ACCOUNTS_URL`, `ZOHO_API_URL`, `ZOHO_API_VERSION`, `ZOHO_LEAD_FIELD_MAP_PATH`, `ALLOW_TEST_SYNC`, and the existing Zoho per-run/per-day budget names, with a cross-field validator requiring them only when lead sync is enabled. Credentials come only from runtime settings populated by Rehaan's SOPS-managed environment; they are never requested, read from a real environment during tests, hardcoded, logged, echoed, stored in the mapping file, or sent in chat.
...
Tests continued: mapping tests cover every configured field, missing optional values, single-word/full names, company-from-domain fallback without exposing the local part, source preservation, exact size/segment rules, unverified/test tags, unknown input exclusion, duplicate/invalid API-name configuration, and outgoing-model-only payloads. Database/job tests use fakes plus fixed-SQL assertions to prove a stable `(created_at, id)` cursor, parameter binding, deterministic order, batch size 25 or less, `FOR UPDATE SKIP LOCKED`, spam exclusion, and test-row exclusion unless `ALLOW_TEST_SYNC=true`. Job paths prove: empty page; success and live cursor advance; two rows with the same timestamp ordered by id; concurrent runs issue one HTTP write through the atomic audit claim; prior `ok`/pending claims skip; transient failure does not advance beyond the failed row; restart retries safely; budget exhaustion; kill switch; and per-item isolation. The required dry-run test asserts exactly one `integration_log` row with `(zoho, synthetic-supabase-id, upsert_lead, skipped)`, zero OAuth/Zoho HTTP calls, no live-cursor advance, and a redacted intended-write log line. Tests also capture every new log event and assert that synthetic names, email local parts, credentials, note/message text, request bodies, and response bodies are absent. `make check` must pass with no skips/xfails and at least 85% coverage for `src/tis/mapping` and `src/tis/jobs`.
